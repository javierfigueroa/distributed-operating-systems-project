...
public class Connection implements Runnable {
	...
	private void listen() throws IOException, InterruptedException {
		if (Client.simulator++ % 2 == 0) {		
			...
			Log.write("Simulating a lost request", Client.log);
		}
	}
	...
	private int read(StringTokenizer st) throws InterruptedException {
		...
		if (PropertyManager.getProperties().containsKey("Client"+this.id+".sleep")) {
			sleepTime = Integer.parseInt(PropertyManager.getProperties().get("Client"+this.id+".sleep"));
		}
		
		Log.write("I HAVE THE TOKEN!",	Client.log);
		Log.write("Received vector: " + vector + " and queue: " + queue , Client.log);
		Log.write("Queue now is: " + Client.token.getQueue().toString(), Client.log);
		Log.write("Vector now is: " + Client.token.getVector().toString(), Client.log);
