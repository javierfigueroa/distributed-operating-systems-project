...
public class Unicaster implements Runnable {
	...
	public Unicaster(int id) throws NumberFormatException, IOException {
		...
		Log.write("Client has started on port " + this.socket.getLocalPort() + "!", Client.log);
		Log.write("Host name: " + this.host, Client.log);		
	...
	public void run() {
		try {			
			while(true){
				...
				Log.write("Server: Accepting a new connection...", Client.log);
