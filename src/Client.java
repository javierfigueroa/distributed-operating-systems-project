...
public class Client {
	...
	private void work() throws InterruptedException, NumberFormatException, IOException {
		...
		if (this.id == 1) {
			Log.write("Client 1 setting hasToken", Client.log);
		...
		try {
			Log.write("Starting multicast socket in port: " + multicastPort, Client.log);
		...
		Log.write("Out of the loop...", Client.log);
	...
	public static volatile int simulator;
}
